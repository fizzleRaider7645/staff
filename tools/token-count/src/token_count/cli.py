...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.as_json:
        print(json.dumps(_as_json(args.model, counted, failed, total, args.budget), indent=2))
    else:
        _render(args.model, counted, failed, total, args.top, args.budget)
...
def _as_json(model, counted, failed, total, want_budget) -> dict:
    payload: dict = {
        "model": model,
        "total_tokens": total,
...
def _render(model, counted, failed, total, top, want_budget) -> None:
