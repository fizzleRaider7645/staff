...
Or as a Claude Code plugin, from the marketplace this repo publishes:
...
The plugin carries `skill/SKILL.md`, which tells Claude when to reach for the
tool and runs `bin/run` from the plugin directory. `staff publish token-count`
regenerates the plugin after a change.
